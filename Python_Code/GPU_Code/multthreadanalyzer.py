...
def getImagesFromEnsemblePart(dirname, ensemble, part, r_=r_):
    ...
    for i in range(no_of_snapshot_in_part):
        ...
        v_ = gridPointsVelocities(r_,tree,vel[:,:,i])
...
#         for dirs in range(4):
#             getImagesFromEnsemblePart('simulation_data_v_%d' % (dirs+1), ensemble,part)
...
no_of_particles_in_plot = 1000

particle_ids = np.arange(1000)# np.random.randint(0,1000,no_of_particles_in_plot)
particle_ids.sort()
...
for dir_i in range(len(dirs)):
    ...
    for i in range(len(particle_ids)):
        ...
        # ax.plot(newdata)
        # plt.show()
        newvariable = np.fft.fft(newdata)

        

        # print('newdata\n',newdata)
        # print('newdata shape\n',newdata.shape)
        # freq_ = np.fft.fftfreq(newdata.shape[-1])
        
        # ft = np.fft.ifftshift(ft)
        # print('plotting data',x_data_.shape)
        # ax.plot(freq_[idx_] , np.abs(ft)[idx_], label='$p_i = %d$'% particle_ids[i])
        # umodby2 = (np.abs(x_ft)**2 + np.abs(y_ft)**2)/2
        # umodby2 = 
        # print('umodby2',umodby2)

        plotdata += np.abs(newvariable)**2 
        # ax = fig.add_subplot(1,2, 2)
        # f, P = signal.periodogram(newdata)
        # plt.loglog(f, P)
        # ax.set_ylim(bottom=10**-4)
        # plt.show()
        # plt.close()
    freq = np.fft.fftfreq(newdata.shape[-1])
    idx_ =  np.argsort(freq)
    plt.loglog(freq[idx_], plotdata[idx_]/no_of_particles_in_plot)
